...
#include "Actor/Include/Sensors/GPSIsar.h"
...
void AGPSIsar::BeginPlay()
{
	Super::BeginPlay();
	if (TimeSampling != 0.0f) {
		SetTickMode(false);
		FTimerHandle ObservationTimerHandle;
		GetWorld()->GetTimerManager().SetTimer(ObservationTimerHandle, this, &AGPSIsar::TakePeriodicObs, TimeSampling, true);
	}
	InitSensor();
	
}

void AGPSIsar::TakePeriodicObs()
{
	TakeObs();
}

// Called every frame
void AGPSIsar::Tick(float DeltaTime)
{
...
const uint32 AGPSIsar::GetObservationSize()
{
	return 48;
}

const bool AGPSIsar::InitSensor()
{
...
const bool AGPSIsar::ChangeSensorSettings(const TArray<FString>& Settings)
{
	if (Settings.Num() == 1)
	{
		TimeSampling = FCString::Atof(*Settings[0]);
		return true;
	}
	else
	{
...
const bool AGPSIsar::GetLastObs(uint8* Buffer)
{
	while (bIsBusy) {}
	if (LastObservation) {
		bIsBusy = true;
...
const FString AGPSIsar::GetSensorName()
{
	return "GPSIsar("+GetActorLabel() + ")";
}

const void AGPSIsar::SetTickMode(bool Value)
{
